...
class Model(UNet):
    def __init__(self, img_shape, T_total, *args, **kwargs):
        assert len(img_shape) == 3, "img_shape should be (C, H, W)"
        assert img_shape[1] == img_shape[2], "img_shape H and W should be the same"
        
        super().__init__(img_channels=img_shape[0], img_size=img_shape[1], *args, **kwargs)
    ...
    def sample(self, num_samples, time_scheduler):
        ...
        for t in reversed(range(self.T_total)):
            t_batch = torch.full((imgs.size(0),), t, device=imgs.device, dtype=torch.long)
            ...
            alpha_bar_t_prev = time_scheduler.alpha_bar(t_batch - 1)[:, None, None, None]
